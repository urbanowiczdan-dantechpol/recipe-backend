...
export async function parseRecipe(content: string, sourceUrl: string): Promise<Recipe> {
  try {
    ...
    const completion = await openai.chat.completions.create({
      model: 'gpt-4o-mini', // Lub 'gpt-4o' dla lepszej jakości
      messages: [
        { 
          role: 'system', 
      ...
      temperature: 0.1, // Niska = mniej halucynacji, bardziej deterministyczne
    ...
    if (!recipe.title || recipe.title.trim().length === 0) {
      throw new Error('Nie znaleziono tytułu przepisu - sprawdź czy URL zawiera przepis');
    }
    
    if (!recipe.ingredients || recipe.ingredients.length === 0) {
      throw new Error('Nie znaleziono składników - sprawdź czy URL zawiera pełny przepis');
    }
    
    if (!recipe.steps || recipe.steps.length === 0) {
      throw new Error('Nie znaleziono kroków przygotowania');
    }
    ...
    recipe.total_time = recipe.total_time || (recipe.prep_time + recipe.cook_time);
